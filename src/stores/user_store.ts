        ...
        if (err instanceof AxiosError) {
          if (err.response) {
            ...
            return {
              success: true,
              message:
                'Ha ez az email cím regisztrálva van, elküldtünk egy jelszóvisszaállító linket.',
            }
          } else if (err.request) {
            return {
              success: false,
          ...
          if (err.response) {
            ...
            if (status === 400) {
              return {
                success: false,
                message:
                  'Érvénytelen vagy lejárt token! Kérjük, kezdje újra a jelszó visszaállítási folyamatot.',
              }
            } else if (status === 404) {
              return {
                success: false,
