...
export default defineComponent({
  name: "ProfileCouponViewerComponent",
  data() {
    return {
  ...
  methods: {
    async loadCoupons() {
      ...
      try {
        ...
        // Betöltjük a discount type részleteket minden kuponhoz
        await this.loadDiscountTypeDetails()
      } catch (error) {
        this.backendError = "Hiba történt a kuponok betöltésekor"
      }
    },
    async loadDiscountTypeDetails() {
      try {
        // Minden kuponhoz lekérjük a discount type részleteit, ha még nincs betöltve
        const updatedCoupons = []
        for (const coupon of this.coupons) {
          if (!coupon.discount_type_details && typeof coupon.discount_type === 'number') {
            const discountTypeDetails = await this.couponStore.getDiscountTypeById(coupon.discount_type)
            // Új objektumot hozunk létre a reaktivitás érdekében
            updatedCoupons.push({
              ...coupon,
        ...
        // Frissítjük a store-ban a kuponokat
        this.couponStore.coupons = updatedCoupons
      } catch (error) {
        console.error("Hiba a kedvezménytípusok betöltésekor:", error)
      }
    },
              ...
              <td>
                <div class="type-cell">
                  <img
                    ...
                    class="type-image"
                  />
                  <span class="type-name">{{ coupon.discount_type_details?.name || '-' }}</span>
                </div>
              </td>
              <td>
